...
static void input(tetris_game_t *g) {
    uint8_t c = getch();
    switch (c) {
    case 'h': // right
        if (!collision(g, -2, 0, 0))
            g->current.j -= 2;
        break;
    case 'j': // down
    ...
    case 32: // spacebar/drop
        while (!collision(g, 0, 2, 0))
            g->current.i += 2;
        break;
    case 'p': // pause
        // unlock game_mutex to not keep tick_thread waiting on it
        pthread_mutex_unlock(&game_mutex);

        // lock pause_mutex and set paused to 1 so tick_thread waits at correct place
        pthread_mutex_lock(&pause_mutex);
        paused = 1;
        pthread_mutex_unlock(&pause_mutex);

        // wait for user to stop pause
        while (1) {
            uint8_t p = getch();
            if (p == 'q') {
                g->game_over = 1;
        ...
        // lock pause_mutex and signal tick_thread to continue
        pthread_mutex_lock(&pause_mutex);
        pthread_cond_signal(&pause_cond);
        pthread_mutex_unlock(&pause_mutex);

        // put lock back for game_mutex
...
static void *window_loop(void *arg) {
    ...
    double frame_interval = 1.0 / 60; // 60Hz

    nodelay(stdscr, TRUE);
    uint8_t quit = 0;
    while (!quit) {
        clock_gettime(CLOCK_REALTIME, &now);
        double since_previous = (now.tv_sec - previous_frame.tv_sec) + (now.tv_nsec - previous_frame.tv_nsec) / B;

        sleep_time.tv_sec = floor(frame_interval - since_previous);
        sleep_time.tv_nsec = ((frame_interval - since_previous) - sleep_time.tv_sec) * B;
...
static void *tick_loop(void *arg) {
    ...
    while (!game_over) {
        clock_gettime(CLOCK_REALTIME, &now);
        double since_previous = (now.tv_sec - previous_tick.tv_sec) + (now.tv_nsec - previous_tick.tv_nsec) / B;
        sleep.tv_sec = floor(tick_length - since_previous);
        sleep.tv_nsec = ((tick_length - since_previous) - sleep.tv_sec) * B;
